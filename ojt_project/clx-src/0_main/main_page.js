...
function onMenuItemClick(/* cpr.events.CItemEvent */ e){
	...
	new cpr.core.App.load(vsAppId, function(loadedApp) {
		if(vcEmb.getEmbeddedAppInstance()){
			//alert("임베디드 앱에 앱 실행 상태 - > 삭제 합니다 ");
//			if(util.isAppModified(vcEmb.getEmbeddedAppInstance(), "변경사항이 있습니다")){
//				e.preventDefault();
//				alert("변경사항이 있습니다");
//				return false;
//			}
 ...
 */
function onLeft_menuSelectionChange(/* cpr.events.CSelectionEvent */ e){
	/** 
	 * @type cpr.controls.Menu
	 */
	var left_menu = e.control;
	var dataSet = app.lookup("dsMn");
	/** @type cpr.data.DataSet */
	for(var i=0; i<dataSet.getRowCount(); i++){
		if(left_menu.value =='m3'){
//			app.lookup("ea1").style.css({
//				"height" : "880px"
//			})
			return;
		}
